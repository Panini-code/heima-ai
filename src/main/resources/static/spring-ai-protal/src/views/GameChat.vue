    ...
    <div class="game-container">
      ...
      <div v-if="!isGameStarted" class="game-start">
        ...
        <div class="input-area">
          <textarea
            v-model="negotiationScenario"
            placeholder="请输入谈判场景（可选，如：薪资谈判、采购价格谈判）..."
            rows="3"
          ></textarea>
      ...
      <div v-else class="chat-main">
        ...
        <div class="game-stats">
          <div class="stat-item">
            <span class="label">
              <ScaleIcon class="scale-icon" :class="{ 'beating': persuasion >= 100 }" />
              谈判说服力
            </span>
            <div class="progress-bar">
              <div 
                class="progress" 
                :style="{ width: `${persuasion}%` }"
                :class="{ 
                  'low': persuasion < 30,
                  'medium': persuasion >= 30 && persuasion < 70,
                  'high': persuasion >= 70 
                }"
              ></div>
            </div>
            <span class="value">{{ persuasion }}%</span>
          </div>
          <div class="stat-item">
      ...
      <!-- 游戏结束提示 -->
      <div v-if="isGameOver" class="game-over" :class="{ 'success': persuasion >= 100 }">
...
import { PaperAirplaneIcon, ScaleIcon } from '@heroicons/vue/24/outline'
...
const startGame = async () => {
  ...
  persuasion.value = 0  // 重置说服力
  
  // 发送开始谈判请求
  const startPrompt = negotiationScenario.value 
    ? `开始谈判，场景：${negotiationScenario.value}`
    : '随机生成谈判场景，并开始谈判'
  ...
  try {
    ...
    while (true) {
      try {
        ...
        // 尝试从回复中提取说服力
        const persuasionMatch = accumulatedContent.match(/说服力[:：]\s*(\d+)/i)
        if (persuasionMatch) {
          const newPersuasion = parseInt(persuasionMatch[1])
          if (!isNaN(newPersuasion)) {
            persuasion.value = Math.min(100, Math.max(0, newPersuasion))
            
            // 检查谈判胜利或失败条件
            if (persuasion.value >= 100) {
              isGameOver.value = true
              gameResult.value = '恭喜你！谈判成功，对方接受了你的方案！🎉'
            } else if (persuasion.value <= 0) {
              isGameOver.value = true
              gameResult.value = '谈判失败：对方态度强硬，拒绝让步！😔'
          }
        }
        }
        // 更新消息内容
    ...
    if (currentRound.value >= MAX_ROUNDS) {
      isGameOver.value = true
      if (persuasion.value >= 100) {
        gameResult.value = '恭喜你！在最后一轮成功达成协议！🎉'
      } else {
        gameResult.value = `谈判结束：对话轮次已达上限(${MAX_ROUNDS}轮），当前说服力为${persuasion.value}，很遗憾未能达成协议`
      }
    }
    // 检查是否谈判结束
    else if (accumulatedContent.includes('谈判结束')) {
      isGameOver.value = true
